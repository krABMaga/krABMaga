use crate::ant::Ant;
use crate::ants_grid::AntsGrid;
use crate::obstacles_grid::ObstaclesGrid;
use crate::sites_grid::SitesGrid;
use crate::static_objects::StaticObjectType;
use crate::to_food_grid::ToFoodGrid;
use crate::to_home_grid::ToHomeGrid;
...
pub struct State {
    ...
    pub food_source_found: bool,
    pub food_returned_home: bool,
}
...
impl State {
    pub fn new(width: i64, height: i64) -> State {
        State {
            ...
            food_source_found: false,
            food_returned_home: false,
        }
    }
    ...
    pub fn set_food_pheromone(&mut self, loc: &Int2D, value: f64) {
    ...
    pub fn set_home_pheromone(&mut self, loc: &Int2D, value: f64) {
    ...
    pub fn get_obstacle(&self, loc: &Int2D) -> Option<StaticObjectType> {
        self.obstacles_grid.grid.get_value_at_pos(loc)
    }

    /// Set an obstacle in a particular grid cell.
    pub fn set_obstacle(&mut self, loc: &Int2D) {
        self.obstacles_grid
            .grid
            .set_value_at_pos(loc, StaticObjectType::OBSTACLE);
    }

    /// Set the location of an ant to a particular cell.
    pub fn set_ant_location(&mut self, ant: &mut Ant, loc: &Int2D) {
    ...
    pub fn get_site(&self, loc: &Int2D) -> Option<StaticObjectType> {
        self.sites_grid.grid.get_value_at_pos(loc)
    }

    /// Set a particular site in a grid cell.
    pub fn set_site(&mut self, loc: &Int2D, site: StaticObjectType) {
        self.sites_grid.grid.set_value_at_pos(loc, site);
    }

    /// Evaporate the pheromones of the simulation.
    pub fn trigger_evaporation(&mut self) {
        self.to_home_grid.multiply(EVAPORATION);
        self.to_food_grid.multiply(EVAPORATION);
    }
}

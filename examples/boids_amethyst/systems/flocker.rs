...
use abm::{
    field_2d::toroidal_distance, field_2d::toroidal_transform, field_2d::Field2D, location::Real2D,
};
use amethyst::{
    core::Transform,
...
pub const AVOIDANCE: f64 = 1.;
pub const RANDOMNESS: f64 = 1.;
pub const CONSISTENCY: f64 = 1.;
pub const MOMENTUM: f64 = 1.;
...
impl FlockerSystem {
    /// Generates a Real2D with random coordinates.
    ...
    /// Takes a single agent and a list of his neighbors and calculates avoidance, cohesion and
    /// consistency Real2Ds, along with returning a randomized Real2D and the momentum of the agent.
    fn process_neighbors(
        agent: &AgentAdapter,
        vec: &Vec<AgentAdapter>,
    ) -> (Real2D, Real2D, Real2D, Real2D, Real2D) {
        let count = vec.len();
        ...
        for other in vec.iter() {
            if agent != other {
                ...
                avoidance.x += dx / (square * square + 1.);
                avoidance.y += dy / (square * square + 1.);
                if !other.dead {
                    cohesion.x += dx;
                    cohesion.y += dy;
        ...
        if count > 0 {
            avoidance.x = 400. * avoidance.x / count as f64;
            avoidance.y = 400. * avoidance.y / count as f64;
...
impl<'s> System<'s> for FlockerSystem {
    // Specify what data we are going to operate on and in which way.
    // Amethyst will give it to us and it will build an optimized execution schedule to parallelize
    // systems as much as possible.
    // In this specific case, this system and FPSSystem can be parallelized because they access different resources.
    type SystemData = (
    ...
    fn run(&mut self, (mut transforms, mut agent_adapters, mut field): Self::SystemData) {
        for (agent_adapter, transform) in (&mut agent_adapters, &mut transforms).join() {
            ...
            let rotation = if last_d.x == 0. || last_d.y == 0. {
                0.
            } else {
                last_d.y.atan2(last_d.x)
            };
            let rotation = (rotation - (PI * 0.5)) as f32;
            ...
            transform.set_translation_xyz(new_x as f32, new_y as f32, 0.);
        }
    }
}

...
impl<'s> System<'s> for AntSystem {
    ...
    fn run(&mut self, (mut transforms, mut agent_adapters, mut ants_grid, obstacles_grid, sites_grid, mut to_food_grid, mut to_home_grid): Self::SystemData) {
        for(agent, transform) in (&mut agent_adapters, &mut transforms).join() {
            agent.deposit_pheromone(&mut to_home_grid, &mut to_food_grid);

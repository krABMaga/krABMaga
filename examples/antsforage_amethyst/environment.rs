...
impl SimpleState for Environment {
	fn on_start(&mut self, data: StateData<'_, GameData<'_, '_>>) {
		...
		let to_food_grid = ToFoodGrid::new(WIDTH, HEIGHT);
		let to_home_grid = ToHomeGrid::new(WIDTH, HEIGHT);

		initialize_camera(world);
		initialize_static_objects(world, sprite_render.clone(), &mut sites_grid, &mut obstacles_grid);
...
// TODO: obstacles
fn initialize_static_objects(world: &mut World, sprite_render: SpriteRender, sites_grid: &mut SitesGrid, _obstacles_grid: &mut ObstaclesGrid) {
	...
	let tint = Tint(Srgba::new(255., 0., 0., 1.));
	world.create_entity()
...
fn initialize_ants(world: &mut World, sprite_render: SpriteRender, ants_grid: &mut AntsGrid) {
	...
	//let debug_tint = Tint(Srgba::new(0., 1., 0., 1.));
	...
	for ant_id in 0..NUM_AGENT{
		...
		/* DEBUG
		if ant_id == 0 {
			transform.set_scale(Vector3::new(4.,4.,1.));
		}*/
		...
		world
			.create_entity()
			.with(sprite_render.clone())
			//.with(if ant_id == 0 {debug_tint}else{global_tint})
			.with(global_tint)
			.with(transform)
			.with(agent_adapter)
			.build();

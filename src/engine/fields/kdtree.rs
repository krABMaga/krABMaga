...
impl<O: Clone + Copy + PartialEq> Kdtree<O> {
    ...
    fn split_on_median(
        ...
        match dir {
            Axis::Vertical => {
                ...
                node_x = median;
                println!(
                    "self.pos_x {}, median vale {}, self.width {}, n1.width {}",
                    self.pos_x, median, self.width, n1.width
                );
            ...
            Axis::Horizontal => {
                ...
                node_y = median;
                println!(
                    "self.pos_y {}, median vale {}, self.height {}, n1.height {}",
                    self.pos_y, median, self.height, n1.height
                );
        ...
        println!(
            "La divisione ha generato l'albero {} in {};{} con w: {} e h:{} e is_leaf: {}",
            n1.id, n1.pos_x, n1.pos_y, n1.width, n1.height, n1.is_leaf
        );
        println!(
            "La divisione ha generato l'albero {} in {};{} con w: {} e h:{} e is_leaf: {}",
            n2.id, n2.pos_x, n2.pos_y, n2.width, n2.height, n2.is_leaf
        );
        return (n1, n2);
    }
    ...
    fn balance(
        &self,
        agents: &mut Vec<(O, i32, i32)>,
        direction: bool,
    ) -> (Vec<Vec<(O, i32, i32)>>, Vec<i32>) {
        let len = agents.len();
...
impl<O: Eq + Clone + Copy> Field for Kdtree<O> {
    fn lazy_update(&mut self) {
        unsafe { std::ptr::swap(&mut self.locs, &mut self.rlocs) }
        self.locs.clear();
        if !self.is_leaf {

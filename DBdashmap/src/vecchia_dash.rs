...
pub struct DashMap<K, V, S = RandomState> {
    ...
    r_shard: std::collections::HashMap<K, V, S>,
    hasher: S,
}
...
impl<K: Eq + Hash + Clone, V: Clone, S: Clone> Clone for DashMap<K, V, S> {
    fn clone(&self) -> Self {
        ...
        let r_shard = self.r_shard.clone();
...
impl<'a, K: 'a + Eq + Hash + Clone, V: 'a + Clone> DashMap<K, V, RandomState> {
...
impl<'a, K: 'a + Eq + Hash + Clone, V: 'a + Clone, S: BuildHasher + Clone> DashMap<K, V, S> {
 

    pub fn update(&mut self) -> std::collections::HashMap<K,V,S>{
        let shard_amount = shard_amount();

        for i in 0..shard_amount{
            let shard = self.shards[i].get();

            self.r_shard.extend(shard.iter().map( | (k,v)| (k.clone(),v.get().clone()) ));
    ...
    pub fn with_capacity_and_hasher(mut capacity: usize, hasher: S) -> Self {
        ...
        let r_shard = std::collections::HashMap::with_hasher(hasher.clone());
...
impl<'a, K: 'a + Eq + Hash + Clone, V: 'a + Clone, S: 'a + BuildHasher + Clone> Map<'a, K, V, S>
    for DashMap<K, V, S>
{
    ...
    fn _get<Q>(&'a self, key: &Q) -> Option<&'a V>
    where
        K: Borrow<Q>,
        Q: Hash + Eq + ?Sized,
    {
        self.r_shard.get(key)
    }
...
impl<'a, K: 'a + Eq + Hash + Clone, V: 'a + Clone, S: BuildHasher + Clone> Shl<(K, V)> for &'a DashMap<K, V, S> {
...
// impl<'a, K: 'a + Eq + Hash+ Clone, V: 'a+ Clone, S: BuildHasher + Clone, Q> Shr<&Q> for &'a DashMap<K, V, S>
// where
//     K: Borrow<Q>,
//     Q: Hash + Eq + ?Sized,
// {
...
// impl<'a, K: 'a + Eq + Hash + Clone, V: 'a + Clone, S: BuildHasher + Clone, Q> BitOr<&Q> for &'a DashMap<K, V, S>
// where
//     K: Borrow<Q>,
//     Q: Hash + Eq + ?Sized,
// {
...
// impl<'a, K: 'a + Eq + Hash + Clone, V:'a + Clone, S: BuildHasher + Clone, Q> Sub<&Q> for &'a DashMap<K, V, S>
// where
//     K: Borrow<Q>,
//     Q: Hash + Eq + ?Sized,
// {
...
// impl<'a, K: 'a + Eq + Hash + Clone, V:'a + Clone, S: BuildHasher + Clone, Q> BitAnd<&Q> for &'a DashMap<K, V, S>
// where
//     K: Borrow<Q>,
//     Q: Hash + Eq + ?Sized,
// {
...
impl<K: Eq + Hash + Clone, V: Clone, S: BuildHasher + Clone> Extend<(K, V)> for DashMap<K, V, S> {
...
impl<K: Eq + Hash + Clone, V: Clone> FromIterator<(K, V)> for DashMap<K, V, RandomState> {
...
mod tests {
    ...
    fn test_different_hashers_randomstate() {
        ...
        for i in 0..10 {
            ...
            assert_eq!(i, *dm_hm_default.get(&i).unwrap());
        }
    }
}
